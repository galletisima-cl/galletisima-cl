...
import { readCartCount, writeCartCount } from "../../../lib/cart";
...
export default function ProductPage(){
  ...
  const addToCart=()=>{ if(!product)return; if(parseSizes(product.size).length&&!selectedSize){setNotice("Selecciona una medida antes de agregar al carrito.");return;} writeCartCount(readCartCount()+quantity); setNotice(`${quantity} × ${product.name}${selectedSize?` · ${selectedSize}`:""} agregado al carrito`); };
