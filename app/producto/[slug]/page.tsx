...
export default function ProductPage(){
  ...
  const addToCart=()=>{ if(!product)return; if(parseSizes(product.size).length&&!selectedSize){setNotice("Selecciona una medida antes de agregar al carrito.");return;} setNotice(`${quantity} × ${product.name}${selectedSize?` · ${selectedSize}`:""} agregado al carrito`); };
  if(loading)return <main className="product-page product-state"><span className="catalog-loader"><i/> Cargando producto…</span></main>;
  if(notFound||!product)return <main className="product-page product-state"><Image src="/galletisima-logo.png" alt="Galletísima" width={240} height={94}/><h1>Producto no disponible</h1><p>Este producto no existe o ya no está publicado.</p><Link className="button primary" href="/?ver=todos#catalogo">Volver al catálogo</Link></main>;
  const sizes=parseSizes(product.size);
  return <main className="product-page"><header className="product-header shell"><Link href="/" aria-label="Volver al inicio"><Image src="/galletisima-logo.png" alt="Galletísima" width={260} height={102} priority/></Link><Link className="product-back" href="/?ver=todos#catalogo">← Volver al catálogo</Link></header><section className="product-detail shell"><div className={`product-detail-image ${product.image_url?"has-image":""}`} style={{backgroundImage:product.image_url?`url(${product.image_url})`:undefined}} role="img" aria-label={`Imagen de ${product.name}`}/><div className="product-detail-content"><p className="product-sku">SKU {product.sku}</p><h1>{product.name}</h1><strong className="product-detail-price">{product.price?currency.format(product.price):"Consultar"}</strong><p className="product-description">{product.description||"Molde Galletísima creado para dar vida a tus ideas."}</p>{sizes.length?<fieldset className="size-selector"><legend>Selecciona una medida</legend><div>{sizes.map((size)=><button type="button" key={size} className={selectedSize===size?"selected":""} aria-pressed={selectedSize===size} onClick={()=>setSelectedSize(size)}>{size}</button>)}</div></fieldset>:<p className="size-pending">Medida por confirmar</p>}<div className="stock-line"><span className={product.stock>0?"available":"unavailable"}/>{product.stock>0?`${product.stock} unidades disponibles`:"Producto por encargo"}</div><div className="purchase-row"><div className="quantity-picker" aria-label="Cantidad"><button type="button" aria-label="Disminuir cantidad" onClick={()=>setQuantity((value)=>Math.max(1,value-1))}>−</button><strong>{quantity}</strong><button type="button" aria-label="Aumentar cantidad" onClick={()=>setQuantity((value)=>value+1)}>+</button></div><button className="product-add" type="button" onClick={addToCart}>Agregar al carrito</button></div><div className="product-assurances"><span>♡ Diseñado con cariño</span><span>▣ Envíos a todo Chile</span></div></div></section>{notice&&<div className="toast" role="status">{notice}</div>}</main>;
}
