  ...
  return (
    <main>
      ...
      <header className="header shell">
        ...
        {menuOpen && <div className="drawer-layer"><button className="drawer-backdrop" aria-label="Cerrar categorías" onClick={closeMobileMenu} /><nav id="mobile-category-drawer" className="category-drawer" aria-label="Categorías"><div className="drawer-head"><div><small>Explora la tienda</small><strong>Categorías</strong></div><button aria-label="Cerrar categorías" onClick={closeMobileMenu}>×</button></div><form className="drawer-search" onSubmit={(event) => { event.preventDefault(); setProductSearch(menuSearch); setCatalogPage(1); closeMobileMenu(); document.getElementById("catalogo")?.scrollIntoView({behavior:"smooth"}); }}><label htmlFor="home-drawer-search">Buscar productos o categorías</label><div><input id="home-drawer-search" type="search" value={menuSearch} onChange={(event) => setMenuSearch(event.target.value)} placeholder="¿Qué molde buscas?" autoComplete="off"/><button type="submit" aria-label="Buscar productos">⌕</button></div></form><MobileCategoryAccordions categories={visibleMenuCategories} navigation={navigationConfig} close={closeMobileMenu} />{!visibleMenuCategories.length && <p className="drawer-search-empty">No hay categorías con ese nombre. Presiona buscar para revisar los productos.</p>}</nav></div>}
      </header>
